...
def load_data():
    data = pd.read_csv('flood data 1.csv')
    return data
...
st.write("This app predicts the floods in tamilnadu based on rainfall data.")
...
st.write("accuracy:", accuracy_score(y_test, rfc.predict(X_test)))
...
if st.button("Predict"):
    ...
    #st.write(f"Prediction: {dtc_prediction}")
    st.write(f"Prediction: {rfc_prediction}")
...
# Load the dataset
df = pd.read_csv('flood data 1.csv')
